...
// editor row: store a line of text as a pointer to the dynamically allocated char data
typedef struct erow
{
    int size;
    char *chars;
...
// there is no dynamic strings in C, so create our own string type that supports appending
struct abuf
{
    char *b;
    int len;
